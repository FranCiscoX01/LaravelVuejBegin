            ...
            <div class="bkng-tb-cntnt">
                <div class="pymnts">
                    <form action="#" method="POST" id="payment-form">
                        <input type="hidden" v-model="token_id" name="token_id" id="token_id">
                            ...
                            <div class="pymnt-cntnt">
                                ...
                                <div class="sctn-row">
                                    <div class="sctn-col l">
                                        ...
                                        <input v-model="name" type="text" placeholder="Como aparece en la tarjeta" autocomplete="off" data-openpay-card="holder_name">
                                    </div>
                                    <div class="sctn-col">
                                ...
                                <div class="sctn-row">
                                        <a class="button rght" @click="pay()" id="pay-button">Pagar</a>
                                </div>
                            </div>
                        </div>
                    </form>
...
export default {
    ...
    mounted() {
        OpenPay.setId('mtcva1yje2dweultsugn');
        OpenPay.setApiKey('pk_f0b9037bb2634974931a7061b8cfd31c');
    ...
    methods: {
        pay(){
            OpenPay.token.extractFormAndCreate('payment-form',
                (response) => { if(response.status == 200) { console.log(response); alert("bien") } },
                (error) => { console.log(error); }
            );
            axios.post('api/charge', {
                    name: this.name,
                    deviceIdHiddenFieldName: this.token_id,
                }).then((response) => {
...
.pymnt-cntnt div.sctn-row div.sctn-col.cvv {
    background-image: url("/public/images/cvv.png");
...
.openpay div.logo {
    background-image: url("/public/images/openpay.png");
...
.openpay div.shield {
    background-image: url("/public/images/security.png");
    background-position: left bottom;
    background-repeat: no-repeat;
    font-size: 12px;
    font-weight: 400;
...
.card-expl div.credit {
    background-image: url("/public/images/cards1.png");
    border-right: 1px solid #ccc;
    margin-left: 30px;
    width: 209px;
}
